...
import { apiUpFans, apiUpList, UpSql } from "@/api/up";
...
export default defineComponent({
  name: "UpList", components: { BUpLink, BTable, BImage },
  props: {
    query: String
  },
  setup() {
    ...
    const { run, result } = useRequest(() => apiUpList(page.value, 30, sort.value, orderby.value))
    return {
      result,
      run,
      page,
